...
function CoinHistory() {
  ...
  if (isLoading) {
    return <p style={{ marginTop: '20px' }}>Fetching 10-year history for {coinId}...</p>;
  }

  if (error) {
    return <p style={{ color: 'red', marginTop: '20px' }}>Error fetching history: {error}</p>;
  }
  
  if (!historyData || historyData.length === 0) {
    return <p style={{ marginTop: '20px' }}>No historical data available for {coinId}.</p>;
  }
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '15px' }}>
        ...
        <tbody>
          {historyData.map((item) => (
            ...
            <tr key={item.id}>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>**{formatDate(item.timestamp)}**</td>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>{formatPrice(item.open)}</td>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>{formatPrice(item.close)}</td>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>{formatPrice(item.high)}</td>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>{formatPrice(item.low)}</td>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>{item.totalVolume.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
